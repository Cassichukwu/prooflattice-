...
contract DemosthenesArena is Ownable, ReentrancyGuard {
    ...
    modifier onlyTaskAgent(uint256 roundId) {
        ...
        require(registry.identityRegistry().ownerOf(r.taskAgentId) == msg.sender, "Not task agent operator");
        _;
    }
    ...
    function judge(
        ...
        require(registry.identityRegistry().ownerOf(judgeAgentId) == msg.sender, "Not judge operator");
    ...
    function settle(uint256 roundId) external nonReentrant {
        ...
        ProofLatticeRegistry.AgentMetadata memory m = registry.agentMeta(r.taskAgentId);
        int256 newScore = int256(uint256(m.trustScore)) + int256(trustDelta);
    ...
    function _selectJury(uint256 excludeAgentId, uint256 n) internal view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < total * 2 && found < n; i++) {
            if (idx > total) idx = 1;
            ProofLatticeRegistry.AgentMetadata memory m = registry.agentMeta(idx);
            if (m.active && m.trustScore >= MIN_TRUST_TO_JUDGE && idx != excludeAgentId) {
                // Avoid duplicates
