...
contract XBountyBoard is Ownable, ReentrancyGuard {
    ...
    function fileBounty(
        ...
        ProofLatticeRegistry.AgentMetadata memory m = registry.agentMeta(accusedAgentId);
        require(m.active, "Agent not active");
    ...
    function resolveGuilty(uint256 bountyId, uint16 newAgentScore) external {
        ...
        // Slash the agent
        ProofLatticeRegistry.AgentMetadata memory m = registry.agentMeta(b.accusedAgentId);
        if (newAgentScore < m.trustScore) {
